...
for play in plays._plays:
    gameId = play.game_id
    if (gameId == 1219 or gameId == 432 or gameId == 862 or gameId == 4355):  # these games have invalid data
        continue
...
def get_rank(game_dict):
    rank_ = game_dict[1]['rank']
    if not rank_:
        return 999999
    return rank_
...
sortedGames = sorted(gameDict.items(), key=lambda x: get_rank(x))
